...
1.  **환각(Hallucination) 방지:** 근거 자료(Context)를 제공하여 사실이 아닌 내용을 지어내는 것을 억제함
2.  **최신 정보 반영:** 학습 시점 이후의 정보도 검색을 통해 답변할 수 있음
3.  **데이터 보안:** 기밀 데이터를 모델에 학습시키지 않고도 보안이 유지된 상태로 검색하여 답변을 생성함

---

## 2. 동작 원리 (Step-by-Step)

1.  **임베딩 (Embedding):** 사용자의 질문을 숫자 벡터(Vector)로 변환함
2.  **검색 (Retrieval):** 벡터 DB에서 질문과 의미가 가장 유사한 문서 조각(Chunk)들을 찾아냄
3.  **프롬프트 구성 (Augmentation):** 검색된 문서 조각을 질문과 합쳐서 "이 문서를 참고해서 답변해줘"라는 프롬프트를 만듬
4.  **답변 생성 (Generation):** LLM이 완성된 프롬프트를 보고 정확한 답변을 생성함

---

## 3. 실무 코드 예시

백엔드 개발자가 실제로 RAG 시스템을 구현할 때의 핵심 로직 (Python/LangChain 기준)

### 1) 문서 인덱싱 (Indexing)
문서를 조각내고 벡터 DB에 저장하는 과정
...
사용자 질문에 대해 검색하고 답변을 생성하는 API 내부 로직
...
PostgreSQL의 `pgvector` 확장을 사용하여 벡터 유사도 검색을 수행하는 SQL
